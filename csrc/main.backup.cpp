...
int main(int argc, char *argv[]) {
	if (argc != 3) {
		std::cout
			<< "SDFGen - A utility for converting closed oriented triangle "
			   "meshes into grid-based signed distance fields.\n";
		std::cout << "\nThe output file format is:";
		std::cout << "<value_1> <value_2> <value_3> [...]\n\n";
		std::cout
			<< "<value_n> are the signed distance data values, in ascending "
			   "order of i, then j, then k.\n";

		std::cout
			<< "The output filename will match that of the input, with the "
		...
		std::cout
			<< "\t<dx> specifies the length of grid cell in the resulting "
			   "distance field.\n";
	...
	if (filename.size() < 5 ||
		...
		std::cerr << "Error: Expected OBJ file with filename of the form "
					 "<name>.obj.\n";
		exit(-1);
	}
	...
	Array3f phi_grid;
	make_level_set3(faceList, vertList, min_box, dx, sizes[0], sizes[1],
					sizes[2], phi_grid);
