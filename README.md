...
C++ core is Orthodox C++ on C++20: C headers (`<assert.h>`, `<math.h>`, `<string.h>`), manual `malloc`/`free`, no allocating STL. `pybind11` boundary (`csrc/pybind.cpp`) stays Modern C++ as an unorthodox island; `Vec3` math keeps `operator+/-/*` by exception; `Array3<T>` templates allowed for generic containers (`T* storage`).
